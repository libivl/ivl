...
#ifndef IVL_QT_GUI_ITEM_VIEWS_SORT_FILTER_PROXY_MODEL_HPP
#define IVL_QT_GUI_ITEM_VIEWS_SORT_FILTER_PROXY_MODEL_HPP
...
class _ivlqt_export_ sort_filter_proxy_model : public QSortFilterProxyModel,
	public method_binder <sort_filter_proxy_model, QSortFilterProxyModel>
{
	Q_OBJECT
	typedef QSortFilterProxyModel T;
...
private:
	...
	void init()
	{
		...
		bind(set_filter_reg_exp)      .to <T, &T::setFilterRegExp>();
		bind(set_filter_wildcard)     .to <&T::setFilterWildcard>();
		bind(set_filter_fixed_string) .to <&T::setFilterFixedString>();
		bind(clear)                   .to <&T::clear>();
		bind(invalidate)              .to <&T::invalidate>();
	}
...
//-----------------------------------------------------------------------------
	...
	// QSortFilterProxyModel
	slot <void (const QString&)> set_filter_reg_exp;
	slot <void (const QString&)> set_filter_wildcard;
	slot <void (const QString&)> set_filter_fixed_string;
	slot <void ()> clear;
	slot <void ()> invalidate;

//-----------------------------------------------------------------------------

	explicit sort_filter_proxy_model(QObject* parent = 0) :
		QSortFilterProxyModel(parent) { init(); }
...
using qt_details::sort_filter_proxy_model;

//-----------------------------------------------------------------------------

}  // namespace ivl

#endif  // IVL_QT_GUI_ITEM_VIEWS_SORT_FILTER_PROXY_MODEL_HPP
