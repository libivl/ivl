...
#ifndef IVL_CORE_DETAILS_TUPLE_TUPLE_HPP
#define IVL_CORE_DETAILS_TUPLE_TUPLE_HPP
...
struct tuple_base : types::tuple_identifier
{
	...
	struct concat_t
	{
        typedef join_tuple_helper<derived_type, TP> jh_class;
		typedef typename jh_class::type j_class;

		typedef typename tuple_helper<
		typename j_class::template val_t<1>::type,
		typename j_class::template val_t<2>::type,
		typename j_class::template val_t<3>::type,
		typename j_class::template val_t<4>::type,
		typename j_class::template val_t<5>::type,
		typename j_class::template val_t<6>::type,
		typename j_class::template val_t<7>::type,
		typename j_class::template val_t<8>::type,
		typename j_class::template val_t<9>::type,
		typename j_class::template val_t<10>::type>::type t_class;

		typedef j_class join_class;

		typedef typename types::t_if<
			types::t_lt<typename j_class::size_value_type,
			types::number<10> >,
			t_class,
			j_class>::type type1;

		//typedef type1 type; // first 10 flat tuple, then list of joined
		typedef join_class type; // always list
	};

	template<class EL>
	struct append_t //: concat_t<const typename tuple_helper<EL>::type> { }; ERROR!!
	{
        typedef join_tuplee_helper<derived_type, EL> jh_class;
		typedef typename jh_class::type j_class;
...
class join_tuplee : public tuple_base<T1::size + 1, join_tuplee<T1, E> >
{
    typedef typename types::t_if<types::is_ref<T1>,
                    T1, const T1&>::type prv_expected_t1;

public:
	typedef tuple_base<T1::size + 1, join_tuplee<T1, E> > tuple_base_type;

	typedef typename core_details::tpl_ref_type<E>::cref je_t;

	struct eval_t
	{
		typedef E type;
		typedef typename types::best_ref<type, E>::type ref;
		typedef typename types::best_ref<type, types::t_true>::type ref_const;
		static ref val(je_t e) { return e; }
		static ref_const val_const(ref_const e) { return e; }
	};

	// members
	T1 t1; // TODO: used to be T1&. Should make others (maybe) behave the same.
	E e;
	...
	struct val_t
	{
		typedef typename types::t_le<
			types::number<N>, typename T1::size_value_type>::type first;
		typedef typename types::t_gt<
			types::number<N-1>, typename T1::size_value_type>::type last;
		typedef core_details::tpl_val<0, E> skip_t;

		typedef typename types::t_if<first,
			typename T1::template val_t<N>,
			typename types::t_if<last, skip_t, eval_t>::type
		>::type val_struct;
		...
		static ref_const val(T& t)
		{
			return val_struct::val_const(types::r_if<first>(t.t1, t.e));
		}
		template <class T>
		static ref_const val_const(const T& t)
		{
	...
	join_tuplee(prv_expected_t1 t1, je_t e) : t1(t1), e(e) { }

	join_tuplee(const join_tuplee& o) : t1(o.t1), e(o.e) { }

	using tuple_base_type::operator =;
	using tuple_base_type::operator ,;

	join_tuplee& operator=(const join_tuplee& o)
	{
		t1 = o.t1;
		e = o.e;
		return *this;
	...
	typedef typename types::t_add<
		typename T1::size_value_type, types::number<1> >
		::type size_value_type;
...
template <class TIN, class T>
struct join_tuplee_helper<TIN, T> : types::t_id<join_tuplee<TIN, T> > { };
...
struct sep_val
{
	typedef sep type;
	typedef sep ref;
	typedef sep ref_const;
	template<class T>
	static inline sep val(T) { return sep(); }
	template<class T>
	static inline sep val_const(T) { return sep(); }
};
...
: public tuple_base<T1::size + T2::size + 1, join_tuple_sep<T1, T2> >
{
	...
	struct accessor
	{
		...
		typedef typename types::t_if<first,
			typename T1::template val_t<N>,
			typename types::t_if<sp, core_details::sep_val,
				typename T2::template val_t<secondidx::value>
				>::type
		>::type val_struct;
	...
	tuple<T1, T2, T3, T4, T5, T6, T7, T8, T9, T10> >
{
	...
	typedef typename core_details::tpl_ref_type<T1>::cref j1;
	typedef typename core_details::tpl_ref_type<T2>::cref j2;
	typedef typename core_details::tpl_ref_type<T3>::cref j3;
	typedef typename core_details::tpl_ref_type<T4>::cref j4;
	typedef typename core_details::tpl_ref_type<T5>::cref j5;
	typedef typename core_details::tpl_ref_type<T6>::cref j6;
	typedef typename core_details::tpl_ref_type<T7>::cref j7;
	typedef typename core_details::tpl_ref_type<T8>::cref j8;
	typedef typename core_details::tpl_ref_type<T9>::cref j9;
	typedef typename core_details::tpl_ref_type<T10>::cref j10;
...
*/
	...
	template<int N>
	typename core_details::tpl_val<N, tuple>::ref
		val(types::number<N> = types::number<N>())
	{
		return core_details::tpl_val<N, tuple>::val(*this);
	}
	template<int N>
	typename core_details::tpl_val<N, const tuple>::ref
		val(types::number<N> = types::number<N>()) const
	{
		return core_details::tpl_val<N, const tuple>::val(*this);
	}

	template<int N>
	struct val_t : public core_details::tpl_val<N, tuple> {};
...
template<class T1>
tuple<
		typename core_details::reslv<T1>::type
	> tpl(T1 t1)
{
	return tuple<
		typename core_details::reslv<T1>::type
...
tuple<
		typename core_details::reslv<T1>::type,
		typename core_details::reslv<T2>::type
	> tpl(T1 t1, T2 t2)
{
	return tuple<
		typename core_details::reslv<T1>::type,
...
tuple<
		typename core_details::reslv<T1>::type,
		typename core_details::reslv<T2>::type,
		typename core_details::reslv<T3>::type
...
tuple<
		...
		typename core_details::reslv<T2>::type,
		typename core_details::reslv<T3>::type,
		typename core_details::reslv<T4>::type
...
tuple<
		...
		typename core_details::reslv<T3>::type,
		typename core_details::reslv<T4>::type,
		typename core_details::reslv<T5>::type
...
tuple<
		...
		typename core_details::reslv<T4>::type,
		typename core_details::reslv<T5>::type,
		typename core_details::reslv<T6>::type
...
tuple<
		...
		typename core_details::reslv<T5>::type,
		typename core_details::reslv<T6>::type,
		typename core_details::reslv<T7>::type
	> tpl(T1 t1,
...
tuple<
		...
		typename core_details::reslv<T6>::type,
		typename core_details::reslv<T7>::type,
		typename core_details::reslv<T8>::type
	> tpl(T1 t1,
...
tuple<
		...
		typename core_details::reslv<T7>::type,
		typename core_details::reslv<T8>::type,
		typename core_details::reslv<T9>::type
	> tpl(T1 t1,
...
tuple<
		...
		typename core_details::reslv<T8>::type,
		typename core_details::reslv<T9>::type,
		typename core_details::reslv<T10>::type
	> tpl(T1 t1,
...
{
	core_details::apply_tuple_impl<T::size>::operate(t, o);
};
template <class T, class O>
...
{
	core_details::apply_tuple_impl<T::size>::output(t, o);
};

} // namespace ivl

#endif // IVL_CORE_DETAILS_TUPLE_TUPLE_HPP
