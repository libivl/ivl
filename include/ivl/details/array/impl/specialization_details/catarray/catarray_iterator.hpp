...
#ifndef IVL_ARRAY_SPECIALIZATION_DETAILS_CATARRAY_ITERATOR_HPP
#define IVL_ARRAY_SPECIALIZATION_DETAILS_CATARRAY_ITERATOR_HPP
#if 0
namespace ivl {
...
template <class T, class IT1, class IT2, bool CONST, int SPECIALIZED = 0>
class catarray_iterator_type
: public data::data_details::
	iter_operator_expander<
		catarray_iterator_type<T, IT1, IT2, CONST, SPECIALIZED>,
			T, CONST, typename IT::reference>
{
private:
	typedef IT1 reference_iterator1;
	typedef IT2 reference_iterator1;

	template <class J, class Y, class Z, bool C, int S>
	friend class catarray_iterator_type;
	...
	friend class data::data_details::iter_operator_expander
		<catarray_iterator_type, T, CONST, typename IT::reference>;
	...
	*/
	...
	typedef typename types::apply_const<typename IT1::reference,
...
public:
	typedef catarray_iterator_type this_type;
	...
	// constructors
	catarray_iterator_type() { }
	catarray_iterator_type(const this_type& it)
	...
	catarray_iterator_type(
		const catarray_iterator_type<T, Y, BEST_IT, false, C, S>& it)
			: ir(it.ir), sizes(it.sizes), size0(it.size0), diffs(it.diffs),
		diff0(it.diff0), step0(it.step0), back(it.back),
		pos(it.pos), pos0(it.pos0) { }
	...
	template<class Y, bool C, int S>
	this_type& operator=(const catarray_iterator_type<T, Y,
						BEST_IT, true, C, S>& it)
	{
		ir = it.ir;
		sizes = it.sizes;
...
class catarray_iterator_type<T, IT, BEST_IT, true, CONST, SPECIALIZED>
: public data::data_details::
	iter_operator_expander<
		catarray_iterator_type<T, IT, BEST_IT, true, CONST, SPECIALIZED>, T, CONST,
...
public:
	...
	this_type& operator=(const catarray_iterator_type<T, J,
						BEST_IT, true, C, S>& it)
	{
		ir = it.ir;
		iter = it.iter;
...
#endif
#endif // IVL_ARRAY_SPECIALIZATION_DETAILS_CATARRAY_ITERATOR_HPP
