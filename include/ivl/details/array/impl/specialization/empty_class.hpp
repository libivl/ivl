...
class array<T, data::empty<DERIVED_INFO> > :
	public array_common_base<array<T, data::empty<DERIVED_INFO> > >
{

private:
	typedef array_common_base<array<
		T, data::empty<DERIVED_INFO> > > common_base_class;
...
public:
	...
	public:
		...
		const T operator [] (size_t j) const { return T(0); }
...
@{*/
	...
	//! do read-only access to any element of an empty array
...
//! returns empty array
//! Used for array_nd<T>::cut()
template<class T>
inline
array<T, data::empty<> > empty()
{
	return array<T, data::empty<> >();
}
