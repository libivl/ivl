...
class array<T, data::ref_val_repeat<DERIVED_INFO> >
	...
	public array_common_base<array<T,
		data::ref_val_repeat<DERIVED_INFO> > >
{

private:
	typedef array_common_base<array<T,
		data::ref_val_repeat<DERIVED_INFO> > > common_base_class;
...
public:
	...
	public:
		...
		const_iterator(size_t i, const T* val) : i(i), val(val) { }
...
@{*/
	...
	array(size_t count, const T& s) : len(count), val(&s) { }
	...
	array(const array<J, S>& a, size_t n) : len(n) { }

	//! This constructor should be unused but has to
	//! be defined for child compatibility.
	template <class J, class S>
	array(const array<J, S>& a) : len(a.length()) { }
...
/**@}*/
	...
	void init(size_t count, const T& s) { len = count; val = &s; }
