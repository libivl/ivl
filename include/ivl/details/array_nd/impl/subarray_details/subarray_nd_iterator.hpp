...
public: //TODO: reset to private and add friend
...
typename types::t_if<types::t_expr<REF_IS_ND && PAST_END_CAPABLE &&
		...
		data::past_end_capable_iterator<IT>, IT>::type ref_iter_type;
*/
public: //TODO: add friend and make protected
	ref_iter_type ptr;
protected:
	size_t step;
	ptrdiff_t posit;
...
// make 2 pointers/iter overhead).
	...
	subarray_nd_iterator(
		...
		const array<size_t, S>& pos) :
			ptr(types::code_word<>(), ptr, np), step(step), posit(posit), idx(idx), diff(diff),
			cur_pos(cur_pos), type_deprecated(type), pos(pos)
		{ }
		
	// new for copying wrapped iter
	template<class S>
	subarray_nd_iterator(
		const ref_iter_type& ptr,
		size_t step,
		ptrdiff_t posit,
		const size_t* idx,
		ptrdiff_t* diff,
		...
		sub_type type,
		const array<size_t, S>& pos) :
			ptr(ptr), step(step), posit(posit), idx(idx), diff(diff),
			cur_pos(cur_pos), type_deprecated(type), pos(pos)
		{ }		
...
IT _begin(A& a, size_t d)
{
	...
	return IT(0, a.get_iter(j), a.steps[j], 0, (*a.idx)[d].c_ptr(), a.diffs[j],
...
//	return IT(it.ptr.begin_pos(), a.steps[j], it.pos[k < 0 ? 0 : k], (*a.idx)[d].c_ptr(),
//		a.diffs[j], k, a.type_from_indtype((*a.idx)[d].indtype), it.pos);

//?? TODO:Maybe if (k >= 0 && k == it.curpos) return IT(it.ptr.begin_pos(), a.steps[j], it.posit, ... ) ???

	return IT(it.ptr, a.steps[j], k < 0 ? 0 : it.pos[k], (*a.idx)[d].c_ptr(),
		a.diffs[j], k, a.type_from_indtype((*a.idx)[d].indtype), it.pos);
...
IT _iter(A& a, size_t d, const ITS& it)
{
	...
	return iterator_nd(IT::tool::wrapper_creation_index_diff(it.ir, a.iter[-3]), 
		IT::tool::wrapper_creation_index_base(it.ir, a.iter[-3]), 
		a.steps[j], it.pos0, (*a.idx)[d].c_ptr(),
		a.diffs[j], k, a.type_from_indtype((*a.idx)[d].indtype), it.pos,
		1, it.get_pos1());
...
IT _iter(A& a, size_t d, const array<size_t, S>& ind)
{
	...
	for(size_t i = 0; i < ind.length(); i++) {
		...
		if(q < 0) {
			IT::tool::move_ref_along(np, it, a.iter[n], a.steps[n], ind[i]);
		} else {
			IT::tool::move_ref_along(np, it, a.iter[n], a.steps[n], (*a.idx)[i][ind[i]]);
			pos[q] = ind[i];
		}
	}
	size_t j = a.nonsing_rmap[d];
	...
	return IT(np, it, a.steps[j], ind[d], (*a.idx)[d].c_ptr(), a.diffs[j],
...
IT _next(A& a, size_t d, const IT& it)
{
	...
	typename IT::ref_iter_type itp = it.ptr;

	size_t j = a.nonsing_rmap[d];
	ptrdiff_t k = a.idx_ar_rmap[d]; // cur_pos

	IT::tool::move_ref_along(itp, a.iter[j], 1, a.begin_to_end[j]);
...
IT _end(A& a, size_t d)
{
	...
	IT::tool::add_to_ref(np, it, a.begin_to_ends[j]);
	//it += a.begin_to_ends[j];

	if(k < 0)
		return IT(np, it, a.steps[j], a.sizes[j], (*a.idx)[d].c_ptr(), a.diffs[j],
			k, a.type_from_indtype((*a.idx)[d].indtype),
							a.idx_ar_map.length());
	else
		return IT(np, it, a.steps[j], a.sizes[j], (*a.idx)[d].c_ptr(), a.diffs[j],
...
IT _last(A& a, size_t d)
{
	...
	IT::tool::add_to_ref(np, it, a.first_to_lasts[j]);

	if(k < 0)
		return IT(np, it, a.steps[j], a.sizes[j] - 1,
