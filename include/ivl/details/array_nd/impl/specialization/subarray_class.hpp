...
private:
	...
	static inline sub_type type_from_indtype(ivl::index_array::
		index_array_type y)
	{
		return array_nd_details::subarray_common_tools::
			type_from_indtype(y);
	}
...
public:
	...
	typedef data::subarray_nd_iterator<T,
			typename base_class::const_reference_iterator,
			typename base_class::const_reference_iterator,
...
*/
	...
	friend IT subarray_iter_create::_begin(X& a, size_t d);
	template <class IT, class X>
	friend IT subarray_iter_create::_begin(X& a, size_t d, const IT& it);
	template <class IT, class ITS, class X>
	friend IT subarray_iter_create::_iter(X& a, size_t d, const ITS& it);
	template <class IT, class S, class X>
	friend IT subarray_iter_create::_iter(X& a, size_t d,
		const array<size_t, S>& ind);
	template <class IT, class X>
	friend IT subarray_iter_create::_next(X& a, size_t d, const IT& it);
	template <class IT, class X>
	friend IT subarray_iter_create::_end(X& a, size_t d);
	template <class IT, class X>
	friend IT subarray_iter_create::_last(X& a, size_t d);

	// non-const iterator
	iterator_nd _begin(size_t d)
	{ return subarray_iter_create::_begin<iterator_nd>(*this, d); }

	iterator_nd _begin(size_t d, const iterator_nd& it)
	{ return subarray_iter_create::_begin<iterator_nd>(*this, d, it); }

	iterator_nd _iter(size_t d, const typename
		array_nd::best_iterator& it)
	{ return subarray_iter_create::_iter<iterator_nd>(*this, d, it); }

	template <class S>
	iterator_nd _iter(size_t d, const array<size_t, S>& ind)
	{ return subarray_iter_create::_iter<iterator_nd>(*this, d, ind); }

	iterator_nd _next(size_t d, const iterator_nd& it)
	{ return subarray_iter_create::_next<iterator_nd>(*this, d, it); }

	iterator_nd _end(size_t d)
	{ return subarray_iter_create::_end<iterator_nd>(*this, d); }

	iterator_nd _last(size_t d)
	{ return subarray_iter_create::_last<iterator_nd>(*this, d); }

	// const iterator
	const_iterator_nd _begin(size_t d) const
	{ return subarray_iter_create::_begin<const_iterator_nd>(*this, d); }

	const_iterator_nd _begin(size_t d, const const_iterator_nd& it) const
	{ return subarray_iter_create::_begin<const_iterator_nd>(*this, d, it); }

	const_iterator_nd _iter(size_t d, const typename
		array_nd::const_iterator& it) const
	{ return subarray_iter_create::_iter<const_iterator_nd>(*this, d, it); }

	template <class S>
	const_iterator_nd _iter(size_t d, const array<size_t, S>& ind) const
	{ return subarray_iter_create::_iter<const_iterator_nd>(*this, d, ind); }

	const_iterator_nd _next(size_t d, const const_iterator_nd& it) const
	{ return subarray_iter_create::_next<const_iterator_nd>(*this, d, it); }

	const_iterator_nd _end(size_t d) const
	{ return subarray_iter_create::_end<const_iterator_nd>(*this, d); }

	const_iterator_nd _last(size_t d) const
	{ return subarray_iter_create::_last<const_iterator_nd>(*this, d); }

	// fast
	_fast_iterator_nd _fast_begin(size_t d)
	{ return subarray_iter_create::_begin<_fast_iterator_nd>(*this, d); }
	_fast_iterator_nd _fast_last(size_t d)
	{ return subarray_iter_create::_last<_fast_iterator_nd>(*this, d); }
	_fast_const_iterator_nd _fast_begin(size_t d) const
	{ return subarray_iter_create::_begin<_fast_const_iterator_nd>(*this, d); }
	_fast_const_iterator_nd _fast_last(size_t d) const
	{ return subarray_iter_create::_last<_fast_const_iterator_nd>(*this, d); }
...
@{*/
	...
	{
		// no stored stride information because subarray has no random access
