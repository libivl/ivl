...
template <class T,
	     ...
	     >
class array_nd<T,
		data::outer<F, A1, A2, DERIVED_INFO> > :
...
public:
	...
	public:
		...
		const_iterator_nd_class(const iter_nd_1& i1, const iter_nd_2& i2)
			: i1(i1), i2(i2) { }
		...
		reference operator [] (ptrdiff_t j) const
		{
			return reference(array_details::elem_func_swap_op<SWAP_ARGS,
				...
				const_iterator_nd_class(i1 + j, i2 + j));
		}
		...
		this_type operator +(const iter_nd_border_walker& z)
			{ this_type tmp(i1 + z.iw1, i2 + z.iw2); return tmp; }
...
//YOU ARE HERE
	...
	{
		return F<T, typename A1::elem_type, typename A2::elem_type>::elem_op(
			in1[indx[0]], in2[indx[1]]);
	}
	//@}


	// TODO: place properly
	void init(const A1& a1, const A2& a2) { this->setref(a1, a2); }
...
/**@{*/
	...
	array_nd(const A1& a1, const A2& a2) { this->setref(a1, a2); }
